...
function automotive_update_notifier_menu() {
	$xml = automotive_get_latest_theme_version(86400); // This tells the function to cache the remote call for 86400 seconds (24 hours)
	if(is_child_theme()){
		global $new_theme_dir_name;

		$theme_data = wp_get_theme( (isset($new_theme_dir_name) && !empty($new_theme_dir_name) ? $new_theme_dir_name : 'automotive') );           // Get theme data from style.css (current version is what we want)
	} else {
	...
	if(isset($xml->latest) && isset($theme_data['Version']) && version_compare($theme_data['Version'], $xml->latest) == -1) {
		...
		add_theme_page( 'Automotive Theme Updates', 'Automotive <span class="update-plugins count-1"><span class="update-count">New Updates</span></span>', 'administrator', 'automotive-updates', 'automotive_update_notifier' );
...
add_action('admin_menu', 'automotive_update_notifier_menu');

function automotive_update_notifier() {
	$xml = automotive_get_latest_theme_version(86400); // This tells the function to cache the remote call for 86400 seconds (24 hours)
	if(is_child_theme()){
		global $new_theme_dir_name;
	...
	<div class="wrap">
        ...
        <div id="instructions" style="max-width: 800px;">
            ...
            <p><strong>Please note:</strong> Make a <strong>backup</strong> of the Theme inside your WordPress installation folder <strong>/wp-content/themes/<?php echo strtolower($theme_data['Name']); ?>/</strong> and your <b>database</b></p>
            <p>To update the Theme, enter your username and API key in the theme options panel. Then click update when you have entered the correct information.</p>
            ...
            <?php } else {
							echo sprintf(__("Verify your purchase under %sTheme Options >> Update Settings%s.", "automotive"), '<a href="' . admin_url('admin.php?page=automotive_wp&tab=15') . '">', '</a>');
						} ?>
        </div>

      <div class="clear"></div>

	    <h3 class="title"><?php esc_html_e("Changelog", "automotive"); ?></h3>
...
function automotive_get_latest_theme_version($interval) {
	// remote xml file location
	$notifier_file_url = 'https://themesuite.com/version/automotive-wp/notifier.xml';
